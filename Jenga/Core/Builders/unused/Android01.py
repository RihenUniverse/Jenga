...
Ajoute la compilation Java, les bibliothèques Java (.jar), les exécutables console,
et le support complet des Android App Bundles (AAB).
"""
...
class AndroidBuilder(Builder):
    ...
    Supporte :
      - Compilation NDK (C/C++)
      - Compilation Java (si sources présentes)
    ...
    # -----------------------------------------------------------------------
    # Compilation native (partiellement modifié pour exécutables)
    # -----------------------------------------------------------------------
    ...
    def Link(self, project: Project, objectFiles: List[str], outputFile: str) -> bool:
        ...
        if project.kind == ProjectKind.STATIC_LIB:
            ...
            if project.kind == ProjectKind.CONSOLE_APP:
                ...
                min_api = project.androidMinSdk or 21
                arch_lib = self.toolchain.sysroot / "usr" / "lib" / self.ndk_triple / str(min_api)
                crtbegin = arch_lib / "crtbegin_dynamic.o"
                crtend = arch_lib / "crtend_android.o"
                if crtbegin.exists():
                    args.insert(args.index("-o") + 2, str(crtbegin))  # juste avant les objects?
                    # En pratique, crtbegin doit être avant les objets, crtend après
                    # On va les placer explicitement
                    # Réorganiser : linker, crtbegin, objets, libs, crtend
                    # Pour simplifier, on les ajoute avant et après
                    args = [linker] + link_type + ["-o", str(out)] + [str(crtbegin)] + final_objects + [str(crtend)] + args[args.index("-o")+3:]
                else:
    ...
    def BuildAPK(self, project: Project, nativeLibs: List[str]) -> bool:
        ...
        # 4. Collecter toutes les sources Java du projet et des dépendances
        java_src_dirs = self._CollectJavaSourceDirs(project)
        java_libs = self._CollectJavaLibraries(project)

        # 5. Compiler le bytecode Java (sources + R.java) et les bibliothèques en DEX
        dex_files = self._CompileDex(project, r_java_dir, java_src_dirs, java_libs, build_dir)
        if dex_files is None:
            return False

        # 6. Assembler l'APK non signé
    ...
    def _CollectJavaSourceDirs(self, project: Project) -> List[Path]:
        """Collecte tous les répertoires contenant des sources Java."""
        dirs = []
        # Répertoire standard java/
        java_dir = Path(self.ResolveProjectPath(project, "java"))
        if java_dir.exists():
            dirs.append(java_dir)
        # Répertoire src/main/java (convention Gradle)
        gradle_java = Path(self.ResolveProjectPath(project, "src/main/java"))
        if gradle_java.exists():
            dirs.append(gradle_java)
        # Attribut personnalisé
        if hasattr(project, 'androidJavaDirs'):
            for d in project.androidJavaDirs:
    ...
    def _CollectJavaLibraries(self, project: Project) -> List[Path]:
        """Collecte les fichiers .jar (dépendances Java)."""
        libs = []
        if hasattr(project, 'androidJavaLibraries'):
            for lib in project.androidJavaLibraries:
                p = Path(self.ResolveProjectPath(project, lib))
                if p.exists() and p.suffix == ".jar":
                    libs.append(p)
        # Ajouter les dépendances des projets liés (si elles ont des .jar)
        for dep_name in project.dependsOn:
            dep = self.workspace.projects.get(dep_name)
            if dep and hasattr(dep, 'androidJavaLibraries'):
                for lib in dep.androidJavaLibraries:
                    p = Path(self.ResolveProjectPath(dep, lib))
                    if p.exists() and p.suffix == ".jar":
                        libs.append(p)
        return libs
    ...
    def _GenerateManifest(self, project: Project, output_dir: Path) -> Path:
        ...
        # Si le projet a des sources Java, on suppose qu'il a une activité Java
        has_java = bool(self._CollectJavaSourceDirs(project))
    ...
    def _CompileDex(self, project: Project, r_java_dir: Path, java_src_dirs: List[Path],
                    java_libs: List[Path], build_dir: Path) -> Optional[List[Path]]:
        """
        Compile les sources Java et les bibliothèques en DEX.
        Retourne la liste des fichiers .dex générés (peut être multiple si multi-dex).
        """
        # Étape 1: Compiler les sources Java en .class avec javac
        classes_dir = build_dir / "classes"
        FileSystem.MakeDirectory(classes_dir)

        # Collecter tous les fichiers .java
        java_files: List[Path] = []
        # Ajouter R.java généré
        r_java_files = list(r_java_dir.rglob("*.java"))
        java_files.extend(r_java_files)
        # Ajouter les sources des répertoires Java
        for src_dir in java_src_dirs:
            java_files.extend(src_dir.rglob("*.java"))

        if not java_files and not java_libs:
            # Aucun code Java, pas de DEX nécessaire
            return []
        ...
        # Étape 2: Convertir les .class et les .jar en DEX avec d8
        dex_out = build_dir / "dex"
        FileSystem.MakeDirectory(dex_out)

        # Collecter tous les .class générés
        ...
        # Ajouter les répertoires de classes
        if class_files:
            d8_cmd.append(str(classes_dir))

        # Si ProGuard/R8 est demandé, on pourrait insérer une étape ici
        if self.use_proguard:
            # TODO: intégrer ProGuard/R8
            Colored.PrintWarning("ProGuard/R8 not implemented yet, skipping")
    ...
    # -----------------------------------------------------------------------
    # Packaging AAB (Android App Bundle) - version complète
    # -----------------------------------------------------------------------
