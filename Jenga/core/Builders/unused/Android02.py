...
class AndroidBuilder(Builder):
    ...
    def BuildAPK(self, project: Project, nativeLibs: List[str]) -> bool:
        ...
        # 4. Collecter toutes les sources Java du projet (via buildOptions)
    ...
    def _CollectJavaSourceFiles(self, project: Project) -> List[Path]:
        """Collecte tous les fichiers .java via les patterns dans buildOptions['androidJavaFiles']."""
        patterns = project.buildOptions.get('androidJavaFiles', [])
        if not patterns:
            return []
        # Résoudre les patterns par rapport à la localisation du projet
    ...
    def _CollectJavaLibraries(self, project: Project) -> List[Path]:
        """Collecte les fichiers .jar via buildOptions['androidJavaLibs']."""
        patterns = project.buildOptions.get('androidJavaLibs', [])
        if not patterns:
            return []
        proj_root = Path(self.ResolveProjectPath(project, "."))
        libs = []
