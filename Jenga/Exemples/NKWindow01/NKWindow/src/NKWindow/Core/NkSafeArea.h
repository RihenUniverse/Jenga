...
struct NkSafeAreaInsets
{
    ...
    NkSafeAreaInsets(float t, float b, float l, float r)
    ...
    { float w = totalWidth  - left - right;  return w > 0 ? static_cast<NkU32>(w) : 0; }

    NkU32 UsableHeight(NkU32 totalHeight) const
    { float h = totalHeight - top  - bottom; return h > 0 ? static_cast<NkU32>(h) : 0; }
    ...
    {
        return x >= left && x <= totalW - right
            && y >= top  && y <= totalH - bottom;
    }

    std::string ToString() const
    {
        return "SafeArea(T=" + std::to_string(top)
...
struct NkSafeAreaData
{
    ...
    NkSafeAreaData(const NkSafeAreaInsets& i, NkU32 w, NkU32 h)
