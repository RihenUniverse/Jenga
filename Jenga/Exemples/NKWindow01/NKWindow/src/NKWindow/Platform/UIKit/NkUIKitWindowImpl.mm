...
NkUIKitWindowImpl::~NkUIKitWindowImpl() { if (mIsOpen) Close(); }
...
bool NkUIKitWindowImpl::Create(const NkWindowConfig& config)
{
    ...
    @autoreleasepool
    {
        ...
        if (ev) ev->Initialize(this, (__bridge void*)mUIView);
    }
    return mIsOpen;
}
...
void NkUIKitWindowImpl::Close()
{
    ...
    {
        IEventImpl* ev = NkGetEventImpl();
        if (ev) ev->Shutdown((__bridge void*)mUIView);
        mUIWindow = nil;
        mUIView   = nil;
        mMetalLayer = nil;
        mIsOpen = false;
...
bool   NkUIKitWindowImpl::IsOpen() const { return mIsOpen; }
NkError NkUIKitWindowImpl::GetLastError() const { return mLastError; }

std::string NkUIKitWindowImpl::GetTitle() const { return mConfig.title; }
void NkUIKitWindowImpl::SetTitle(const std::string& t) { mConfig.title = t; }

void NkUIKitWindowImpl::SetVisible(bool v)
{
    @autoreleasepool { mUIWindow.hidden = !v; }
}
...
{
    @autoreleasepool { return static_cast<float>([UIScreen mainScreen].scale); }
}
...
NkVec2u NkUIKitWindowImpl::GetDisplaySize() const
{
    @autoreleasepool
    {
        ...
        return { static_cast<NkU32>(sz.width * sc),
                 static_cast<NkU32>(sz.height * sc) };
    }
}
