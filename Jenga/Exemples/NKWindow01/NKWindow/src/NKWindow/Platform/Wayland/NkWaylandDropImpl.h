...
public:
    ...
    ~NkWaylandDropImpl()
    {
        if (mDataDevice) { wl_data_device_destroy(mDataDevice); mDataDevice = nullptr; }
    }
...
private:
    ...
    static void OnDataDeviceEnter(void* data, wl_data_device*, uint32_t /*serial*/,
    ...
    {
        auto* self = static_cast<NkWaylandDropImpl*>(data);
        if (self->mDropLeave) self->mDropLeave();
        if (self->mOffer)
        {
    ...
    static void OnDataDeviceMotion(void* data, wl_data_device*, uint32_t,
    ...
    {
        auto* self = static_cast<NkWaylandDropImpl*>(data);
        if (!self->mOffer) return;

        bool hasUri  = false;
        bool hasText = false;
        for (const auto& m : self->mMimeTypes)
        {
            if (m == "text/uri-list") hasUri  = true;
            if (m == "text/plain;charset=utf-8" || m == "text/plain") hasText = true;
        }
        ...
        else if (hasText && self->mDropText)
        {
            const std::string mime =
                (std::find(self->mMimeTypes.begin(), self->mMimeTypes.end(),
