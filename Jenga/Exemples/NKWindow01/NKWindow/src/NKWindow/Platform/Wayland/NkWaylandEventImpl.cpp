...
NkWaylandEventImpl::~NkWaylandEventImpl()
{
    ...
    if (mPointer)    { wl_pointer_destroy(mPointer);   mPointer    = nullptr; }
    if (mKeyboard)   { wl_keyboard_destroy(mKeyboard); mKeyboard   = nullptr; }
    if (mTouch)      { wl_touch_destroy(mTouch);       mTouch      = nullptr; }
}
...
void NkWaylandEventImpl::Initialize(IWindowImpl* owner, void* nativeHandle)
{
    ...
    WindowEntry entry;
    entry.window = win;
    mWindowMap[mWlData->surface] = entry;
...
const NkEvent& NkWaylandEventImpl::Front() const                  { return mQueue.front();  }
void           NkWaylandEventImpl::Pop()                          { mQueue.pop_front();     }
bool           NkWaylandEventImpl::IsEmpty()  const               { return mQueue.empty();  }
std::size_t    NkWaylandEventImpl::Size()     const               { return mQueue.size();   }
void           NkWaylandEventImpl::PushEvent(const NkEvent& evt)  { mQueue.push_back(evt); }
...
void NkWaylandEventImpl::OnKeyboardKeymap(
    void* data, wl_keyboard*, uint32_t fmt, int fd, uint32_t size)
{
    auto* self = static_cast<NkWaylandEventImpl*>(data);
    ...
    if (self->mXkbKeymap) xkb_keymap_unref(self->mXkbKeymap);
    if (self->mXkbState)  xkb_state_unref(self->mXkbState);

    self->mXkbKeymap = xkb_keymap_new_from_string(
        self->mXkbContext, raw,
...
void NkWaylandEventImpl::OnKeyboardKey(
    void* data, wl_keyboard*, uint32_t /*serial*/, uint32_t /*time*/,
    uint32_t key, uint32_t state)
{
    ...
    NkKeyData kd;
    kd.key     = nkKey;
    kd.scancode = static_cast<NkU32>(key);
    kd.state   = (state == WL_KEYBOARD_KEY_STATE_PRESSED)
                     ? NkButtonState::NK_PRESSED
                     : NkButtonState::NK_RELEASED;
    kd.mods    = BuildMods(self->mXkbState);
...
void NkWaylandEventImpl::OnKeyboardMods(
    void* data, wl_keyboard*, uint32_t /*serial*/,
...
void NkWaylandEventImpl::OnPointerEnter(void*, wl_pointer*, uint32_t, wl_surface*,
                                        wl_fixed_t sx, wl_fixed_t sy)
{
    // curseur entré dans la surface — on peut changer le curseur ici
    (void)sx; (void)sy;
}

void NkWaylandEventImpl::OnPointerLeave(void*, wl_pointer*, uint32_t, wl_surface*) {}

void NkWaylandEventImpl::OnPointerMotion(
    void* data, wl_pointer*, uint32_t /*time*/, wl_fixed_t sx, wl_fixed_t sy)
{
    auto* self = static_cast<NkWaylandEventImpl*>(data);
    self->mPointerX = static_cast<NkF32>(wl_fixed_to_double(sx));
    self->mPointerY = static_cast<NkF32>(wl_fixed_to_double(sy));
...
void NkWaylandEventImpl::OnPointerButton(
    void* data, wl_pointer*, uint32_t /*serial*/, uint32_t /*time*/,
    uint32_t button, uint32_t state)
{
    auto* self = static_cast<NkWaylandEventImpl*>(data);

    NkMouseButton btn = NkMouseButton::NK_MOUSE_LEFT;
    if (button == BTN_RIGHT)  btn = NkMouseButton::NK_MOUSE_RIGHT;
    if (button == BTN_MIDDLE) btn = NkMouseButton::NK_MOUSE_MIDDLE;

    NkMouseButtonData bd;
    bd.button = btn;
    bd.x      = self->mPointerX;
    bd.y      = self->mPointerY;
    bd.state  = (state == WL_POINTER_BUTTON_STATE_PRESSED)
                    ? NkButtonState::NK_PRESSED
                    : NkButtonState::NK_RELEASED;
...
void NkWaylandEventImpl::OnPointerAxis(
    void* data, wl_pointer*, uint32_t /*time*/, uint32_t axis, wl_fixed_t value)
{
    ...
    if (axis == WL_POINTER_AXIS_VERTICAL_SCROLL)
        wd.deltaY = static_cast<NkF32>(-v / 10.0);
    else
        wd.deltaX = static_cast<NkF32>(-v / 10.0);
...
// ---------------------------------------------------------------------------
// Conversion XKB keysym → NkKey  (extrait de NkXLibEventImpl, recopié)
// ---------------------------------------------------------------------------

NkKey NkWaylandEventImpl::XkbKeyToNkKey(xkb_keysym_t sym)
{
    // Mapping keysym → NkKey (identique à XLib, les keysyms sont les mêmes)
    switch (sym)
    {
        case XKB_KEY_Escape:       return NkKey::NK_ESCAPE;
        case XKB_KEY_Return:       return NkKey::NK_RETURN;
        case XKB_KEY_Tab:          return NkKey::NK_TAB;
        case XKB_KEY_BackSpace:    return NkKey::NK_BACKSPACE;
        ...
        case XKB_KEY_Page_Up:      return NkKey::NK_PAGEUP;
        case XKB_KEY_Page_Down:    return NkKey::NK_PAGEDOWN;
        ...
        case XKB_KEY_a:
        case XKB_KEY_A:            return NkKey::NK_A;
        case XKB_KEY_b:
        case XKB_KEY_B:            return NkKey::NK_B;
        case XKB_KEY_c:
        case XKB_KEY_C:            return NkKey::NK_C;
        case XKB_KEY_d:
        case XKB_KEY_D:            return NkKey::NK_D;
        case XKB_KEY_e:
        case XKB_KEY_E:            return NkKey::NK_E;
        case XKB_KEY_f:
        case XKB_KEY_F:            return NkKey::NK_F;
        case XKB_KEY_g:
        case XKB_KEY_G:            return NkKey::NK_G;
        case XKB_KEY_h:
        case XKB_KEY_H:            return NkKey::NK_H;
        case XKB_KEY_i:
        case XKB_KEY_I:            return NkKey::NK_I;
        case XKB_KEY_j:
        case XKB_KEY_J:            return NkKey::NK_J;
        case XKB_KEY_k:
        case XKB_KEY_K:            return NkKey::NK_K;
        case XKB_KEY_l:
        case XKB_KEY_L:            return NkKey::NK_L;
        case XKB_KEY_m:
        case XKB_KEY_M:            return NkKey::NK_M;
        case XKB_KEY_n:
        case XKB_KEY_N:            return NkKey::NK_N;
        case XKB_KEY_o:
        case XKB_KEY_O:            return NkKey::NK_O;
        case XKB_KEY_p:
        case XKB_KEY_P:            return NkKey::NK_P;
        case XKB_KEY_q:
        case XKB_KEY_Q:            return NkKey::NK_Q;
        case XKB_KEY_r:
        case XKB_KEY_R:            return NkKey::NK_R;
        case XKB_KEY_s:
        case XKB_KEY_S:            return NkKey::NK_S;
        case XKB_KEY_t:
        case XKB_KEY_T:            return NkKey::NK_T;
        case XKB_KEY_u:
        case XKB_KEY_U:            return NkKey::NK_U;
        case XKB_KEY_v:
        case XKB_KEY_V:            return NkKey::NK_V;
        case XKB_KEY_w:
        case XKB_KEY_W:            return NkKey::NK_W;
        case XKB_KEY_x:
        case XKB_KEY_X:            return NkKey::NK_X;
        case XKB_KEY_y:
        case XKB_KEY_Y:            return NkKey::NK_Y;
        case XKB_KEY_z:
        case XKB_KEY_Z:            return NkKey::NK_Z;
        case XKB_KEY_0:            return NkKey::NK_0;
        case XKB_KEY_1:            return NkKey::NK_1;
        case XKB_KEY_2:            return NkKey::NK_2;
        case XKB_KEY_3:            return NkKey::NK_3;
        case XKB_KEY_4:            return NkKey::NK_4;
        case XKB_KEY_5:            return NkKey::NK_5;
        case XKB_KEY_6:            return NkKey::NK_6;
        case XKB_KEY_7:            return NkKey::NK_7;
        case XKB_KEY_8:            return NkKey::NK_8;
        case XKB_KEY_9:            return NkKey::NK_9;
        case XKB_KEY_Shift_L:
        case XKB_KEY_Shift_R:      return NkKey::NK_LSHIFT;
        case XKB_KEY_Control_L:
        case XKB_KEY_Control_R:    return NkKey::NK_LCTRL;
        case XKB_KEY_Alt_L:
        case XKB_KEY_Alt_R:        return NkKey::NK_LALT;
        case XKB_KEY_Super_L:
        case XKB_KEY_Super_R:      return NkKey::NK_LSUPER;
...
{
    NkModifierState m{};
    m.shift = xkb_state_mod_name_is_active(state, XKB_MOD_NAME_SHIFT, XKB_STATE_MODS_EFFECTIVE);
    m.ctrl  = xkb_state_mod_name_is_active(state, XKB_MOD_NAME_CTRL,  XKB_STATE_MODS_EFFECTIVE);
    m.alt   = xkb_state_mod_name_is_active(state, XKB_MOD_NAME_ALT,   XKB_STATE_MODS_EFFECTIVE);
    m.super = xkb_state_mod_name_is_active(state, XKB_MOD_NAME_LOGO,  XKB_STATE_MODS_EFFECTIVE);
    return m;
}
