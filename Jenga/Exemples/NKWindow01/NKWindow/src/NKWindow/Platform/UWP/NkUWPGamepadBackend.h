...
public:
    ...
    ~NkUWPGamepadBackend() override { Shutdown(); }
    ...
    void Poll() override
    {
        ...
        if (count > NK_MAX_GAMEPADS) count = NK_MAX_GAMEPADS;
    ...
    {
        NkU32 n = 0;
        for (const auto& s : mStates) if (s.connected) ++n;
        return n;
    }
    ...
    void Rumble(NkU32 idx, float low, float high,
                float tl, float tr, NkU32 /*ms*/) override
    {
#if defined(__cplusplus_winrt) || defined(NKENTSEU_PLATFORM_UWP)
        auto gamepads = Gamepad::Gamepads;
        if (idx >= (NkU32)gamepads->Size) return;
        ...
        gp->Vibration = v;
#else
        (void)idx; (void)low; (void)high; (void)tl; (void)tr;
#endif
    }
