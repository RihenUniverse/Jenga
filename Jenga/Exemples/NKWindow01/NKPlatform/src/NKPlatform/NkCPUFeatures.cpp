...
namespace nkentseu {
    namespace platform {
        ...
        SIMDFeatures::SIMDFeatures()
            : hasMMX(false), hasSSE(false), hasSSE2(false)
            , hasSSE3(false), hasSSSE3(false), hasSSE41(false)
            , hasSSE42(false), hasAVX(false), hasAVX2(false)
            , hasAVX512F(false), hasAVX512DQ(false), hasAVX512BW(false)
            , hasAVX512VL(false), hasFMA(false), hasFMA4(false)
            , hasNEON(false), hasSVE(false), hasSVE2(false) {
        }
        
        ExtendedFeatures::ExtendedFeatures()
            : hasAES(false), hasSHA(false), hasRDRAND(false)
            , hasRDSEED(false), hasCLFLUSH(false), hasCLFLUSHOPT(false)
            , hasPREFETCHWT1(false), hasMOVBE(false), hasPOPCNT(false)
            , hasLZCNT(false), hasBMI1(false), hasBMI2(false)
        ...
        void CPUFeatures::DetectVendorAndBrand() {
            ...
            std::memcpy(vendorStr + 0, &ebx, 4);
            std::memcpy(vendorStr + 4, &edx, 4);
            std::memcpy(vendorStr + 8, &ecx, 4);
            vendorStr[12] = '\0';
            
            vendor = core::NkString(vendorStr);
            ...
            if (maxExtended >= 0x80000004) {
                for (i32 i = 0; i < 3; ++i) {
                    ...
                    std::memcpy(brandStr + i * 16 + 0,  &eax, 4);
                    std::memcpy(brandStr + i * 16 + 4,  &ebx, 4);
                    std::memcpy(brandStr + i * 16 + 8,  &ecx, 4);
                    std::memcpy(brandStr + i * 16 + 12, &edx, 4);
                }
                brandStr[48] = '\0';
                brand = core::NkString(brandStr);
            }
            ...
            if (brand.IsEmpty()) {
                brand = core::NkString("ARM Processor");
            }
            
            #else
            
            vendor = core::NkString("Unknown");
            brand = core::NkString("Unknown Processor");
        ...
        void CPUFeatures::DetectTopology() {
            ...
            topology.numLogicalCores = static_cast<i32>(std::thread::hardware_concurrency());
        ...
        core::NkString CPUFeatures::ToString() const {
            char buffer[2048];
            int offset = 0;
            ...
            offset += snprintf(buffer + offset, sizeof(buffer) - offset,
                "\nSIMD Features:\n");
            if (simd.hasSSE)    offset += snprintf(buffer + offset, sizeof(buffer) - offset, "  SSE ");
            if (simd.hasSSE2)   offset += snprintf(buffer + offset, sizeof(buffer) - offset, "SSE2 ");
            if (simd.hasSSE3)   offset += snprintf(buffer + offset, sizeof(buffer) - offset, "SSE3 ");
            if (simd.hasSSE41)  offset += snprintf(buffer + offset, sizeof(buffer) - offset, "SSE4.1 ");
            if (simd.hasSSE42)  offset += snprintf(buffer + offset, sizeof(buffer) - offset, "SSE4.2 ");
            if (simd.hasAVX)    offset += snprintf(buffer + offset, sizeof(buffer) - offset, "AVX ");
            if (simd.hasAVX2)   offset += snprintf(buffer + offset, sizeof(buffer) - offset, "AVX2 ");
            if (simd.hasAVX512F) offset += snprintf(buffer + offset, sizeof(buffer) - offset, "AVX-512F ");
            if (simd.hasFMA)    offset += snprintf(buffer + offset, sizeof(buffer) - offset, "FMA ");
            if (simd.hasNEON)   offset += snprintf(buffer + offset, sizeof(buffer) - offset, "NEON ");
            offset += snprintf(buffer + offset, sizeof(buffer) - offset, "\n");
