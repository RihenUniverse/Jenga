...
// Inclusions nécessaires selon la plateforme
...
#elif defined(NKENTSEU_PLATFORM_ANDROID) || defined(NKENTSEU_PLATFORM_IOS) || defined(NKENTSEU_PLATFORM_WASM)
...
namespace nkentseu {
    ...
    // ===========================================================================
        ...
        // en chaîne pour OPENFILENAME (double null-terminated avec des paires description|pattern)
        static std::string Win32PrepareFilter(const std::string& userFilter)
        {
            if (userFilter.empty() || userFilter == "*.*")
                return "All Files\0*.*\0";

            // On va construire un filtre simple : on prend l'extension et on met un libellé
            // Exemple : "*.png;*.jpg" -> "Image Files (*.png;*.jpg)\0*.png;*.jpg\0"
            std::string result;
            result.reserve(userFilter.size() + 32);

            // Créer une description basée sur l'extension
            result += "Fichiers (";
            result += userFilter;
            result += ")\0";
            result += userFilter;
            ...
            // Remplacer les ';' par des '\0' dans la partie pattern (OPENFILENAME attend des patterns séparés par des '\0')
            // Mais la chaîne doit avoir des '\0' entre chaque pattern et un double '\0' à la fin.
            // Pour simplifier, on ne gère pas les patterns multiples ; on les laisse tels quels,
            // l'utilisateur peut passer "*.png;*.jpg" et ça fonctionnera avec l'API Windows?
            // En réalité, OPENFILENAME attend une liste de patterns séparés par ';' dans une seule chaîne, donc "*.png;*.jpg" est correct.
            // On ajoute juste un double null à la fin.
            result.push_back('\0'); // déjà un null de la fin de la chaîne précédente, mais on en ajoute un pour doubler
            return result;
        }
    ...
    // ===========================================================================
        ...
        inline void NkDialogs::OpenMessageBox(const std::string& message, const std::string& title, int type)
        {
            ...
            switch (type) {
                ...
                case 2: cmd += "error"; break;
                default: cmd += "info"; break;
            }
            cmd += " --text=\"";
            cmd += message;
            cmd += "\" --title=\"";
        ...
        inline NkDialogResult NkDialogs::ColorPicker(NkU32 initial)
        {
            ...
            // Le format de sortie est généralement #RRGGBB
    ...
    // ===========================================================================
        ...
        inline void NkDialogs::OpenMessageBox(const std::string& message, const std::string& title, int type)
        {
            ...
            switch (type) {
                case 1: script += " with icon caution"; break;
                case 2: script += " with icon stop"; break;
                default: script += " with icon note"; break;
            }
            script += "'";
            system(script.c_str());
        }
    ...
    // ===========================================================================
        ...
        inline NkDialogResult NkDialogs::OpenFileDialog(const std::string&, const std::string&) { return {}; }
        inline NkDialogResult NkDialogs::SaveFileDialog(const std::string&, const std::string&) { return {}; }
        inline void NkDialogs::OpenMessageBox(const std::string&, const std::string&, int) {}
        inline NkDialogResult NkDialogs::ColorPicker(NkU32) { return {}; }
