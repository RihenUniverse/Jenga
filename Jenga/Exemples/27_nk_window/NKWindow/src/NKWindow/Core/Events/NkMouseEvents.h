...
struct NkMouseMoveData
{
    ...
    NkMouseMoveData(NkI32 x, NkI32 y, NkI32 sx, NkI32 sy,
                    NkI32 dx = 0, NkI32 dy = 0,
                    NkU32 btns = 0,
                    NkModifierState mods = {})
        : x(x), y(y), screenX(sx), screenY(sy)
        , deltaX(dx), deltaY(dy)
        , buttonsDown(btns), modifiers(mods)
    {}
    ...
    {
        return "MouseMove(" + std::to_string(x) + "," + std::to_string(y)
...
struct NkMouseRawData
{
    ...
    NkMouseRawData(NkI32 dx, NkI32 dy, NkI32 dz = 0)
        : deltaX(dx), deltaY(dy), deltaZ(dz) {}

    std::string ToString() const
    {
        return "MouseRaw(dx=" + std::to_string(deltaX)
...
struct NkMouseButtonData
{
    ...
    NkMouseButtonData(NkMouseButton btn, NkButtonState st,
                      NkI32 x, NkI32 y,
                      NkI32 sx = 0, NkI32 sy = 0,
                      NkModifierState mods = {},
                      NkU32 clicks = 1)
        : button(btn), state(st), modifiers(mods)
        , x(x), y(y), screenX(sx), screenY(sy)
        , clickCount(clicks)
    {}
...
struct NkMouseWheelData
{
    ...
    NkMouseWheelData(double dy, double dx = 0.0,
                     NkI32 cx = 0, NkI32 cy = 0,
                     NkModifierState mods = {},
                     bool hiPrecision = false)
        : delta(dy), deltaX(dx), deltaY(dy)
        , x(cx), y(cy)
        , modifiers(mods)
        , highPrecision(hiPrecision)
    {}

    bool IsVertical()   const { return deltaY != 0.0; }
    bool IsHorizontal() const { return deltaX != 0.0; }
    bool ScrollsUp()    const { return deltaY >  0.0; }
    bool ScrollsDown()  const { return deltaY <  0.0; }
    bool ScrollsLeft()  const { return deltaX <  0.0; }
    bool ScrollsRight() const { return deltaX >  0.0; }
