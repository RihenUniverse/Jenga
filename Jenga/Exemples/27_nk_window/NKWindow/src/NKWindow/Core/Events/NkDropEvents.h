...
struct NkDropFilePath
{
    ...
    explicit NkDropFilePath(const char* p) { strncpy(path, p, sizeof(path)-1); }
...
struct NkSystemPowerData
{
    ...
    {
        const char* names[] = {
            "NORMAL","LOW_BATTERY","CRITICAL_BATTERY","PLUGGED_IN","SUSPENDED","RESUMED"
        };
...
struct NkSystemDisplayData
{
    ...
    enum class Change : NkU32 {
        Added, Removed, ResolutionChanged, OrientationChanged, DpiChanged
    ...
    std::string ToString() const
    {
        return "SystemDisplay(#" + std::to_string(displayIndex)
...
struct NkCustomData
{
    ...
    {
        static_assert(sizeof(T) <= NK_CUSTOM_DATA_MAX_BYTES,
                      "NkCustomData payload trop petit");
        std::memcpy(payload, &value, sizeof(T));
        dataSize = sizeof(T);
    }

    template<typename T>
    bool GetPayload(T& out) const
    {
    ...
    std::string ToString() const
    {
        return "CustomEvent(type=" + std::to_string(customType)
