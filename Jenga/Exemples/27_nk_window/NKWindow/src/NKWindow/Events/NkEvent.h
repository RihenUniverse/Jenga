...
#include "NkWindowEvents.h"
#include "NkKeyboardEvents.h"
#include "NkMouseEvents.h"
#include "NkTouchEvents.h"
#include "NkGamepadEvents.h"
#include "NkDropEvents.h"
...
public:
	...
	explicit NkEvent(NkEventType t, Window *w = nullptr)
		: type(t), category(NkGetEventCategory(t)), window(w), timestamp(CurrentTimestamp()) {
	}
	...
	explicit NkEvent(const NkWindowStateData &d, Window *w = nullptr)
		: type(StateToType(d.state)), category(NkEventCategory::NK_CAT_WINDOW), window(w),
	...
	explicit NkEvent(const NkKeyData &d, Window *w = nullptr)
		: type(d.repeat
				   ? NkEventType::NK_KEY_REPEAT
				   : (d.state == NkButtonState::NK_PRESSED ? NkEventType::NK_KEY_PRESS : NkEventType::NK_KEY_RELEASE)),
	...
	explicit NkEvent(const NkMouseWheelData &d, Window *w = nullptr)
		: type(d.deltaX != 0.0 ? NkEventType::NK_MOUSE_WHEEL_HORIZONTAL : NkEventType::NK_MOUSE_WHEEL_VERTICAL),
		  category(NkEventCategory::NK_CAT_MOUSE), window(w), timestamp(CurrentTimestamp()) {
		data.mouseWheel = d;
	}
	...
	explicit NkEvent(const NkGamepadButtonData &d, Window *w = nullptr)
		: type(d.state == NkButtonState::NK_PRESSED ? NkEventType::NK_GAMEPAD_BUTTON_PRESS
	...
	explicit NkEvent(const NkDropFileData &d, Window *w = nullptr)
		: type(NkEventType::NK_DROP_FILE), category(NkEventCategory::NK_CAT_DROP), window(w),
		  timestamp(CurrentTimestamp()) {
	...
	explicit NkEvent(const NkDropTextData &d, Window *w = nullptr)
		: type(NkEventType::NK_DROP_TEXT), category(NkEventCategory::NK_CAT_DROP), window(w),
		  timestamp(CurrentTimestamp()) {
	...
	explicit NkEvent(const NkDropImageData &d, Window *w = nullptr)
		: type(NkEventType::NK_DROP_IMAGE), category(NkEventCategory::NK_CAT_DROP), window(w),
		  timestamp(CurrentTimestamp()) {
	...
	bool IsValid() const {
		return type != NkEventType::NK_NONE;
	}
	...
	bool IsWindow() const {
		return NkCategoryHas(category, NkEventCategory::NK_CAT_WINDOW);
	}
	bool IsKeyboard() const {
		return NkCategoryHas(category, NkEventCategory::NK_CAT_KEYBOARD);
	}
	bool IsMouse() const {
		return NkCategoryHas(category, NkEventCategory::NK_CAT_MOUSE);
	}
	bool IsTouch() const {
		return NkCategoryHas(category, NkEventCategory::NK_CAT_TOUCH);
	}
	bool IsGamepad() const {
		return NkCategoryHas(category, NkEventCategory::NK_CAT_GAMEPAD);
	}
	bool IsDrop() const {
		return NkCategoryHas(category, NkEventCategory::NK_CAT_DROP);
	}
	bool IsSystem() const {
		return NkCategoryHas(category, NkEventCategory::NK_CAT_SYSTEM);
	}
	...
	static NkTimestampMs CurrentTimestamp() {
		...
		return static_cast<NkTimestampMs>(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - sStart).count());
	}
...
private:
	...
	static NkEventType StateToType(NkWindowStateData::State s) {
		switch (s) {
			case NkWindowStateData::State::Minimized:
				return NkEventType::NK_WINDOW_MINIMIZE;
			case NkWindowStateData::State::Maximized:
				return NkEventType::NK_WINDOW_MAXIMIZE;
			case NkWindowStateData::State::Restored:
				return NkEventType::NK_WINDOW_RESTORE;
			case NkWindowStateData::State::Fullscreen:
				return NkEventType::NK_WINDOW_FULLSCREEN;
			case NkWindowStateData::State::Windowed:
				return NkEventType::NK_WINDOW_WINDOWED;
			default:
...
// ===========================================================================
// Macro pour forcer la copiabilité des classes d'événements typés
// ===========================================================================

// Cette macro doit être utilisée dans TOUTES les classes d'événements typées
// qui héritent de NkEvent. Elle rend explicite que la classe est copiable via
// l'héritage des constructeurs de copie/mouvement de NkEvent.
#define NK_ENSURE_COPYABLE(EventClassName)                                                         \
    /* Héritage explicite de tous les constructeurs de la classe parente */                        \
    using NkEvent::NkEvent;                                                                        \
    /* Forcer la génération des copy/move constructors (pas de suppression implicite) */           \
    EventClassName(const EventClassName &other) : NkEvent(static_cast<const NkEvent &>(other)) {} \
    EventClassName &operator=(const EventClassName &other) {                                       \
        NkEvent::operator=(static_cast<const NkEvent &>(other));                                   \
        return *this;                                                                               \
    }                                                                                               \
    EventClassName(EventClassName &&other) noexcept                                                \
        : NkEvent(static_cast<NkEvent &&>(other)) {}                                               \
    EventClassName &operator=(EventClassName &&other) noexcept {                                   \
        NkEvent::operator=(static_cast<NkEvent &&>(other));                                        \
        return *this;                                                                               \
    }
