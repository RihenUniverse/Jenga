...
class NkSoftwareRendererImpl : public NkIRendererImpl
{
public:
    ...
    bool Init(const NkRendererConfig& config,
              const NkSurfaceDesc& surface)  override;
    ...
    const NkU8* GetPixelBuffer()  const { return mBuffer.data(); }
    NkU8*       GetPixelBuffer()        { return mBuffer.data(); }
