...
public:
    ...
    ~NkWin32GamepadBackend() override { Shutdown(); }
    ...
    {
        if (idx >= XUSER_MAX_COUNT) return;
        XINPUT_VIBRATION v{};
        v.wLeftMotorSpeed  = static_cast<WORD>(
            std::min(motorLow,  1.f) * 65535.f);
        v.wRightMotorSpeed = static_cast<WORD>(
            std::min(motorHigh, 1.f) * 65535.f);
...
private:
    ...
    static void FillState(const XINPUT_GAMEPAD& xp, NkGamepadStateData& s)
    {
        ...
        // Axes
        auto ax = [&](A a, float v) {
            s.axes[static_cast<NkU32>(a)] = v;
        };

        ax(A::NK_GP_AXIS_LX,
           NkXI_ApplyDeadzone(xp.sThumbLX, XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE));
        ax(A::NK_GP_AXIS_LY,
           NkXI_ApplyDeadzone(xp.sThumbLY, XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE));
        ax(A::NK_GP_AXIS_RX,
           NkXI_ApplyDeadzone(xp.sThumbRX, XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE));
        ax(A::NK_GP_AXIS_RY,
           NkXI_ApplyDeadzone(xp.sThumbRY, XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE));
    ...
    static void FillBattery(DWORD idx, NkGamepadStateData& s)
    {
        ...
        if (XInputGetBatteryInformation(idx,
            BATTERY_DEVTYPE_GAMEPAD, &bi) != ERROR_SUCCESS) return;
    ...
    void FillInfo(DWORD idx)
    {
        ...
        std::snprintf(info.id,          sizeof(info.id),
                      "XInput#%u",      static_cast<unsigned>(idx));
        std::snprintf(info.vendor.name, sizeof(info.vendor.name),
                      "Xbox Controller %u", static_cast<unsigned>(idx));
    }
};
