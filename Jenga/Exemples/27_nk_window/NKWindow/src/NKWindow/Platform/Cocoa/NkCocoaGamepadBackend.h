...
public:
    ...
    ~NkCocoaGamepadBackend() override { Shutdown(); }
    ...
    void Rumble(NkU32 idx, float motorLow, float motorHigh,
                float triggerLeft, float triggerRight, NkU32 durationMs) override;
...
{
    for (auto& s : mStates) s.connected = false;
}
...
inline void NkCocoaGamepadBackend::Rumble(NkU32 idx, float motorLow, float motorHigh,
...
static void NkGC_FillState(GCController* ctrl, NkU32 idx,
                            NkGamepadStateData& s, NkGamepadInfo& info)
{
    s.connected    = true;
    s.gamepadIndex = idx;
    ...
    if ([cat containsString:@"Xbox"])       info.type = NkGamepadType::NK_GP_TYPE_XBOX;
    else if ([cat containsString:@"Dual"])  info.type = NkGamepadType::NK_GP_TYPE_PLAYSTATION;
    else                                    info.type = NkGamepadType::NK_GP_TYPE_GENERIC;
...
bool NkCocoaGamepadBackend::Init()     { for(auto& s:mStates)s={}; return true; }
void NkCocoaGamepadBackend::Shutdown() { for(auto& s:mStates)s.connected=false; }
...
void NkCocoaGamepadBackend::Rumble(NkU32 idx, float l, float h,
                                    float, float, NkU32)
{
    (void)idx; (void)l; (void)h;
...
void NkCocoaGamepadBackend::SetLEDColor(NkU32 idx, NkU32 rgba)
{
    @autoreleasepool
    {
        ...
        {
            NSArray<GCController*>* ctrls = [GCController controllers];
            if (idx >= (NkU32)ctrls.count) return;
            float r=((rgba>>24)&0xFF)/255.f, g=((rgba>>16)&0xFF)/255.f,
                  b=((rgba>>8)&0xFF)/255.f;
            [ctrls[idx].light
                setColor:[NSColor colorWithRed:r green:g blue:b alpha:1.f]];
        }
    }
}
