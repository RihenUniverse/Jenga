...
public:
    ...
    explicit NkXLibDropImpl(Display* conn, Window win)
        : mConn(conn), mWin(win)
    {
        InitAtoms();
        SetXdndAware();
...
private:
    ...
    {
        xcb_intern_atom_cookie_t c = xcb_intern_atom(mConn, 0,
    ...
    void SetXdndAware()
    {
        ...
        xcb_change_property(mConn, XCB_PROP_MODE_REPLACE,
            mWin, aXdndAware, XCB_ATOM_ATOM, 32, 1, &version);
    }
    ...
    void SendStatus(bool accept)
    {
        ...
        xcb_send_event(mConn, 0, mSourceWin, XCB_EVENT_MASK_NO_EVENT,
    ...
    {
        xcb_convert_selection(mConn, mWin, aXdndSelection,
                              type, aXdndSelection, XCB_CURRENT_TIME);
        xcb_flush(mConn);
    }
...
inline void NkXLibDropImpl::HandleClientMessage(const xcb_client_message_event_t* ev)
{
    if (ev->type == aXdndEnter)
    {
        mSourceWin = static_cast<Window>(ev->data.data32[0]);
        NkDropEnterData d;
        d.x = 0; d.y = 0;
...
inline void NkXLibDropImpl::HandleSelectionNotify(const xcb_selection_notify_event_t* ev)
{
    if (!mHasDrop || ev->property == XCB_NONE) { SendFinished(); return; }

    // Lire la propriété
    xcb_get_property_cookie_t c = xcb_get_property(
        mConn, 1, mWin, ev->property, XCB_GET_PROPERTY_TYPE_ANY, 0, 0x7FFFFFFF);
    xcb_get_property_reply_t* r = xcb_get_property_reply(mConn, c, nullptr);
    if (!r) { SendFinished(); return; }
