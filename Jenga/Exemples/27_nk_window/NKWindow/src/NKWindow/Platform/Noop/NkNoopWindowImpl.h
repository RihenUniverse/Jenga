...
public:
    ...
    {
        mConfig = cfg; mOpen = true;
        auto* ev = NkGetEventImpl();
        if (ev) ev->Initialize(this, reinterpret_cast<void*>(this));
        return true;
    }
    void Close() override
    {
        auto* ev = NkGetEventImpl();
        if (ev) ev->Shutdown(reinterpret_cast<void*>(this));
        mOpen = false;
    }
    ...
    void SetFullscreen(bool fs)          override { mConfig.fullscreen = fs; }
    ...
    {
        NkSurfaceDesc sd;
        sd.width = mConfig.width; sd.height = mConfig.height; sd.dpi = 1.f;
