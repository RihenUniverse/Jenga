...
public:
    ...
    explicit NkWASMDropImpl(const char* targetId = "#canvas")
        : mTargetId(targetId ? targetId : "#canvas")
    {
#ifdef __EMSCRIPTEN__
        ...
        EM_ASM({
            ...
            target.addEventListener('drop', function(e) {
                ...
                if (text) Module._NkWASMOnDropText(text, e.clientX, e.clientY);
            });
            target.addEventListener('dragenter', function(e) {
                Module._NkWASMOnDragEnter(e.clientX, e.clientY);
            });
    ...
    {
        if (!sInstance || !sInstance->mDropFiles) return;
        NkDropFileData d;
        d.x = x; d.y = y; d.numFiles = static_cast<NkU32>(numFiles);
    ...
    {
        if (!sInstance || !sInstance->mDropText) return;
        NkDropTextData td;
        td.x = x; td.y = y;
        td.text = text ? text : "";
    ...
    {
        if (!sInstance || !sInstance->mDropEnter) return;
        NkDropEnterData d; d.x = x; d.y = y;
        sInstance->mDropEnter(d);
    }

    static void OnDragLeave()
    { if (sInstance && sInstance->mDropLeave) sInstance->mDropLeave(); }
...
#ifdef __EMSCRIPTEN__
extern "C" {
    EMSCRIPTEN_KEEPALIVE void NkWASMOnDrop(int n, int x, int y) { NkWASMDropImpl::OnDrop(n, x, y); }
    EMSCRIPTEN_KEEPALIVE void NkWASMOnDropText(const char* t, int x, int y) { NkWASMDropImpl::OnDropText(t, x, y); }
    EMSCRIPTEN_KEEPALIVE void NkWASMOnDragEnter(int x, int y) { NkWASMDropImpl::OnDragEnter(x, y); }
    EMSCRIPTEN_KEEPALIVE void NkWASMOnDragLeave() { NkWASMDropImpl::OnDragLeave(); }
}
#endif
