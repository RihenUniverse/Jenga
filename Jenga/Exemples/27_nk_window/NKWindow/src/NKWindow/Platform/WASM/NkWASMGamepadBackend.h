...
{
public:
    bool Init() override { for(auto&s:mStates)s={}; return true; }
    ...
    void Poll() override {
        ...
        for (int i=0; i<n && (NkU32)i<NK_MAX_GAMEPADS; ++i) {
            ...
            auto sa=[&](A a,int j){ if(j<ev.numAxes)    mStates[i].axes[static_cast<NkU32>(a)]=ev.axis[j]; };
            // Standard mapping W3C
            sb(B::NK_GP_SOUTH,       0); sb(B::NK_GP_EAST,        1);
            sb(B::NK_GP_WEST,        2); sb(B::NK_GP_NORTH,       3);
            sb(B::NK_GP_LB,          4); sb(B::NK_GP_RB,          5);
            sb(B::NK_GP_LT_DIGITAL,  6); sb(B::NK_GP_RT_DIGITAL,  7);
            sb(B::NK_GP_BACK,        8); sb(B::NK_GP_START,       9);
            sb(B::NK_GP_LSTICK,     10); sb(B::NK_GP_RSTICK,     11);
            sb(B::NK_GP_DPAD_UP,    12); sb(B::NK_GP_DPAD_DOWN,  13);
            sb(B::NK_GP_DPAD_LEFT,  14); sb(B::NK_GP_DPAD_RIGHT, 15);
            sb(B::NK_GP_GUIDE,      16);
            sa(A::NK_GP_AXIS_LX,0); sa(A::NK_GP_AXIS_LY,1);
            sa(A::NK_GP_AXIS_RX,2); sa(A::NK_GP_AXIS_RY,3);
            mInfos[i].index=(NkU32)i; mInfos[i].numButtons=(NkU32)ev.numButtons;
