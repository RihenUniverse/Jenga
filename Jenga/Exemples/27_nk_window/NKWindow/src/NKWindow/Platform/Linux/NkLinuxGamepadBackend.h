...
public:
    ...
    ~NkLinuxGamepadBackend() override { Shutdown(); }

    bool Init() override
    {
        for (auto& s : mStates) s = {};
        for (auto& i : mInfos)  i = {};
        for (auto& d : mDevices) { d.fd = -1; d.ffFd = -1; d.ffEffectId = -1; d.open = false; }
    ...
    {
        for (auto& d : mDevices) CloseDevice(d);
                ...
                if (type == JS_EVENT_BUTTON)
                {
                    if (evt.number < static_cast<NkU32>(NkGamepadButton::NK_GAMEPAD_BUTTON_MAX))
                    {
                        ...
                        if (btn != NkGamepadButton::NK_GP_UNKNOWN)
                            mStates[i].buttons[static_cast<NkU32>(btn)] = (evt.value != 0);
                    }
                }
    ...
    void Rumble(NkU32 idx, float motorLow, float motorHigh,
                float /*tl*/, float /*tr*/, NkU32 durationMs) override
    {
        if (idx >= NK_MAX_GAMEPADS) return;
        NkJsDevice& dev = mDevices[idx];
        if (dev.ffFd < 0) return;
        ...
        if (motorLow <= 0.f && motorHigh <= 0.f) return;

        struct ff_effect eff{};
        eff.type                      = FF_RUMBLE;
        eff.id                        = -1;
        eff.u.rumble.strong_magnitude = static_cast<NkU16>(
            std::min(motorLow, 1.f) * 0xFFFF);
        eff.u.rumble.weak_magnitude   = static_cast<NkU16>(
            std::min(motorHigh, 1.f) * 0xFFFF);
        eff.replay.length = durationMs > 0
            ? static_cast<NkU16>(std::min<NkU32>(durationMs, 0xFFFF))
            : 0xFFFF;
        eff.replay.delay  = 0;
...
private:
    ...
    void ScanDevices()
    {
        ...
        {
            if (strncmp(ent->d_name, "js", 2) != 0) continue;
            char path[64];
    ...
    void TryOpenDevice(const char* path)
    {
        ...
        for (NkU32 i = 0; i < NK_MAX_GAMEPADS; ++i)
        {
            if (!mDevices[i].open) { slot = i; break; }
        }
        ...
        strncpy(info.vendor.name, name, sizeof(info.vendor.name) - 1);
    ...
    void TryOpenForceFeedback(NkU32 slot, const char* jsPath)
    {
        ...
        snprintf(sysPath, sizeof(sysPath),
                 "/sys/class/input/%s/device", jsName);
        ...
        while ((ent = readdir(dir)) != nullptr)
        {
            ...
            if (ioctl(fd, EVIOCGBIT(EV_FF, sizeof(ffBits)), ffBits) >= 0)
            {
                ...
                if (hasRumble)
                {
                    ...
                    strncpy(mDevices[slot].eventPath, evPath,
                            sizeof(mDevices[slot].eventPath) - 1);
    ...
    static NkGamepadButton JsButtonToNk(NkU8 n)
    {
        ...
        switch (n)
        {
        case 0:  return B::NK_GP_SOUTH;
        case 1:  return B::NK_GP_EAST;
        case 2:  return B::NK_GP_WEST;
        case 3:  return B::NK_GP_NORTH;
        case 4:  return B::NK_GP_LB;
        case 5:  return B::NK_GP_RB;
        case 6:  return B::NK_GP_BACK;
        case 7:  return B::NK_GP_START;
        case 8:  return B::NK_GP_GUIDE;
        case 9:  return B::NK_GP_LSTICK;
        case 10: return B::NK_GP_RSTICK;
        case 11: return B::NK_GP_DPAD_UP;
        case 12: return B::NK_GP_DPAD_DOWN;
        case 13: return B::NK_GP_DPAD_LEFT;
        case 14: return B::NK_GP_DPAD_RIGHT;
        default: return B::NK_GP_UNKNOWN;
        }
    }
    ...
    static NkGamepadAxis JsAxisToNk(NkU8 n)
    {
        ...
        switch (n)
        {
        case 0: return A::NK_GP_AXIS_LX;
        case 1: return A::NK_GP_AXIS_LY;
        case 2: return A::NK_GP_AXIS_LT;
        case 3: return A::NK_GP_AXIS_RX;
        case 4: return A::NK_GP_AXIS_RY;
