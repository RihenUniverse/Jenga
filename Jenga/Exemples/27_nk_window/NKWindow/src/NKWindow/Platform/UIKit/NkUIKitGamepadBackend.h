...
public:
    ...
    void Shutdown() override { for(auto& s:mStates)s.connected=false; }
    void Poll() override;
...
void NkUIKitGamepadBackend::Poll()
{
    @autoreleasepool {
        ...
        for (GCController* ctrl in ctrls) {
            ...
            auto& s = mStates[idx]; auto& info = mInfos[idx];
            s.connected = true; s.gamepadIndex = idx; info.index = idx;
        ...
        for (; idx < NK_MAX_GAMEPADS; ++idx) mStates[idx].connected = false;
    }
}
}
#endif
