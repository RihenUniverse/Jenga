...
{
    xcb_intern_atom_cookie_t ck = xcb_intern_atom(c, onlyIfExists?1:0,
...
NkXCBWindowImpl::~NkXCBWindowImpl() { if (mData.isOpen) Close(); }
...
bool NkXCBWindowImpl::Create(const NkWindowConfig& config)
{
    ...
    { mLastError = NkError(1,"XCB connection non disponible"); return false; }
    ...
    xcb_change_property(mData.connection, XCB_PROP_MODE_REPLACE, mData.window,
    ...
    xcb_change_property(mData.connection, XCB_PROP_MODE_REPLACE, mData.window,
        mData.wmProtocols, XCB_ATOM_ATOM, 32, 1, &mData.wmDelete);
    ...
    if (config.visible) xcb_map_window(mData.connection, mData.window);
...
void NkXCBWindowImpl::Close()
{
    ...
    if (mData.gc) { xcb_free_gc(mData.connection, mData.gc); mData.gc=0; }
    xcb_flush(mData.connection);
    mData.isOpen = false;
}
...
NkVec2u NkXCBWindowImpl::GetSize() const { return {mData.width,mData.height}; }
...
NkVec2u NkXCBWindowImpl::GetPosition() const {
    ...
    NkVec2u p{};
    if (r) { p={static_cast<NkU32>(r->x),static_cast<NkU32>(r->y)}; free(r); }
    return p;
}
...
void NkXCBWindowImpl::Maximize() {
    ...
    ev.response_type=XCB_CLIENT_MESSAGE; ev.type=st;
    ev.window=mData.window; ev.format=32;
    ev.data.data32[0]=1; ev.data.data32[1]=mh; ev.data.data32[2]=mv;
...
void NkXCBWindowImpl::SetFullscreen(bool fs) {
    ...
    ev.response_type=XCB_CLIENT_MESSAGE; ev.type=st;
    ev.window=mData.window; ev.format=32;
    ev.data.data32[0]=fs?1:0; ev.data.data32[1]=fa;
    xcb_send_event(mData.connection,1,mData.screen->root,
        XCB_EVENT_MASK_SUBSTRUCTURE_REDIRECT|XCB_EVENT_MASK_SUBSTRUCTURE_NOTIFY,
        (const char*)&ev);
    xcb_flush(mData.connection);
...
void NkXCBWindowImpl::SetMousePosition(NkU32 x, NkU32 y) {
    xcb_warp_pointer(mData.connection,XCB_NONE,mData.window,0,0,0,0,
        static_cast<int16_t>(x),static_cast<int16_t>(y));
    xcb_flush(mData.connection);
}

void NkXCBWindowImpl::ShowMouse(bool show) {
    xcb_change_window_attributes(mData.connection,mData.window,
        XCB_CW_CURSOR, show ? (const uint32_t[]){XCB_CURSOR_NONE} : &mData.blankCursor);
    xcb_flush(mData.connection);
}
...
NkSurfaceDesc NkXCBWindowImpl::GetSurfaceDesc() const {
    NkSurfaceDesc sd;
    sd.width=mData.width; sd.height=mData.height;
    sd.connection=mData.connection; sd.window=mData.window;
    return sd;
}

void NkXCBWindowImpl::BlitSoftwareFramebuffer(const NkU8* rgba8, NkU32 w, NkU32 h) {
    if (!mData.connection||mData.window==XCB_WINDOW_NONE||!rgba8) return;
