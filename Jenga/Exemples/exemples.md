...
├── 15_window_win32/          # Fenêtre native Win32
├── 16_window_x11_linux/      # Fenêtre Linux (X11/XCB)
├── 17_window_macos_cocoa/    # Fenêtre macOS Cocoa
├── 18_window_android_native/ # Fenêtre Android NativeActivity
├── 19_window_web_canvas/     # Fenêtre Web Canvas (Emscripten)
├── 20_window_ios_uikit/      # Fenêtre iOS UIKit
├── 21_zig_cross_compile/     # Cross‑compile avec Zig
├── 22_nk_multiplatform_sandbox/ # Sandbox multiplateforme complet
├── 23_android_sdl3_ndk_mk/   # SDL3 Android via Android.mk/ndk-build
...
**Objectif** : démontrer un build Android via `ndk-build` et `Android.mk` avec SDL3 précompilé.

### 📄 `23_android_sdl3_ndk_mk.jenga`
```python
from Jenga import *
...
Le projet inclut un `Android.mk` et `Application.mk` pour le flux `ndk-build`.
...
Jenga build --platform Android-arm64 --target SDL3NativeDemo \
  ...
  --with-sdl3 --sdl3-root "$SDL3_ROOT" \
  --jenga-file 23_android_sdl3_ndk_mk.jenga --no-daemon
```
...
| 19_window_web_canvas    | Fenêtre Web Canvas (Emscripten)                        |
| 20_window_ios_uikit     | Fenêtre iOS UIKit                                      |
| 21_zig_cross_compile    | Cross-compilation avec Zig                             |
| 22_nk_multiplatform_sandbox | Sandbox multi-plateforme                           |
| 23_android_sdl3_ndk_mk  | SDL3 Android via Android.mk / ndk-build                |
