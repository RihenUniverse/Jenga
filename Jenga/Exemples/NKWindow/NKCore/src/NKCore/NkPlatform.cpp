...
// DESCRIPTION: Implémentation des fonctions de détection plateforme runtime
// AUTEUR: Rihen
// DATE: 2026-02-10
// VERSION: 3.0.0
// MODIFICATIONS: Implémentation complète pour NkPlatform.h v3.0.0
// -----------------------------------------------------------------------------
...
static std::atomic<nk_bool> sInitialized(false);

// ============================================================
// FONCTIONS DÉTECTION SIMD - WINDOWS
// ============================================================
...
// ============================================================
// FONCTIONS DÉTECTION SIMD - LINUX
// ============================================================
...
static void NkDetectSIMDLinux() {
        ...
        sPlatformInfo.hasNEON = true; // ARM Linux a généralement NEON
    #endif
}
...
// ============================================================
// FONCTIONS DÉTECTION SIMD - MACOS
// ============================================================
...
// ============================================================
// IMPLÉMENTATION - FONCTION D'INITIALISATION
// ============================================================
...
void NkInitializePlatformInfo() {
    ...
    // Initialiser à zéro
    std::memset(&sPlatformInfo, 0, sizeof(NkPlatformInfo));
    ...
    if (sPlatformInfo.cpuL1CacheSize == 0) sPlatformInfo.cpuL1CacheSize = 32 * 1024;
    if (sPlatformInfo.cpuL2CacheSize == 0) sPlatformInfo.cpuL2CacheSize = 256 * 1024;
    if (sPlatformInfo.cpuL3CacheSize == 0) sPlatformInfo.cpuL3CacheSize = 8 * 1024 * 1024;
...
// ============================================================
// IMPLÉMENTATION - FONCTIONS PUBLIQUES
// ============================================================
...
nk_bool NkHasSIMDFeature(const nk_char* feature) {
    ...
    if (std::strcmp(feature, "SSE") == 0) return info.hasSSE;
    if (std::strcmp(feature, "SSE2") == 0) return info.hasSSE2;
    if (std::strcmp(feature, "SSE3") == 0) return info.hasSSE3;
    if (std::strcmp(feature, "SSE4.1") == 0) return info.hasSSE4_1;
    if (std::strcmp(feature, "SSE4.2") == 0) return info.hasSSE4_2;
    if (std::strcmp(feature, "AVX") == 0) return info.hasAVX;
    if (std::strcmp(feature, "AVX2") == 0) return info.hasAVX2;
    if (std::strcmp(feature, "AVX512") == 0) return info.hasAVX512;
    if (std::strcmp(feature, "NEON") == 0) return info.hasNEON;
...
nk_uint32 NkGetCPUThreadCount() {
        ...
        nk_size size = sizeof(threadCount);
...
nk_uint64 NkGetTotalMemory() {
    ...
    #elif defined(NKENTSEU_PLATFORM_MACOS) || defined(NKENTSEU_PLATFORM_IOS)
        nk_int64 totalMemory = 0;
        nk_size size = sizeof(totalMemory);
...
void NkPrintPlatformInfo() {
    ...
    printf("SIMD Support: ");
    nk_bool first = true;
    if (info.hasSSE) { printf("%sSSE", first ? "" : ", "); first = false; }
    if (info.hasSSE2) { printf("%sSSE2", first ? "" : ", "); first = false; }
    if (info.hasSSE3) { printf("%sSSE3", first ? "" : ", "); first = false; }
    if (info.hasSSE4_1) { printf("%sSSE4.1", first ? "" : ", "); first = false; }
    if (info.hasSSE4_2) { printf("%sSSE4.2", first ? "" : ", "); first = false; }
    if (info.hasAVX) { printf("%sAVX", first ? "" : ", "); first = false; }
    if (info.hasAVX2) { printf("%sAVX2", first ? "" : ", "); first = false; }
    if (info.hasAVX512) { printf("%sAVX-512", first ? "" : ", "); first = false; }
    if (info.hasNEON) { printf("%sNEON", first ? "" : ", "); first = false; }
        ...
        namespace platform {
            namespace memory {
                ...
                nk_ptr NkAllocateAligned(nk_size size, nk_size alignment) noexcept {
                    ...
                    // Vérifier que l'alignement est une puissance de 2
                    ...
                    void* ptr = nullptr;
                        ...
                        if (posix_memalign(&ptr, alignment, size) != 0) {
                            ...
                            // Ajuster la taille pour être multiple de l'alignement
                            if (size % alignment != 0) {
