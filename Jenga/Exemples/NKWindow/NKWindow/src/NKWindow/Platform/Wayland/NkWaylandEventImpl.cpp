...
const NkEvent &NkWaylandEventImpl::Front() const {
	return mQueue.front();
}
...
void NkWaylandEventImpl::PushEvent(const NkEvent &evt) {
...
void NkWaylandEventImpl::OnKeyboardKey(void *data, wl_keyboard *, uint32_t /*serial*/, uint32_t /*time*/, uint32_t key,
									   uint32_t state) {
	auto *self = static_cast<NkWaylandEventImpl *>(data);
	if (!self->mXkbState)
		return;
	...
	kd.state = (state == WL_KEYBOARD_KEY_STATE_PRESSED) ? NkButtonState::NK_PRESSED : NkButtonState::NK_RELEASED;
...
void NkWaylandEventImpl::OnKeyboardMods(void *data, wl_keyboard *, uint32_t /*serial*/, uint32_t mods_dep,
										uint32_t mods_lat, uint32_t mods_lock, uint32_t group) {
	auto *self = static_cast<NkWaylandEventImpl *>(data);
	if (self->mXkbState)
...
void NkWaylandEventImpl::OnPointerButton(void *data, wl_pointer *, uint32_t /*serial*/, uint32_t /*time*/,
	...
	if (button == BTN_RIGHT)
		btn = NkMouseButton::NK_MOUSE_RIGHT;
	if (button == BTN_MIDDLE)
		btn = NkMouseButton::NK_MOUSE_MIDDLE;

	NkMouseButtonData bd;
	bd.button = btn;
	bd.x = self->mPointerX;
	bd.y = self->mPointerY;
	bd.state = (state == WL_POINTER_BUTTON_STATE_PRESSED) ? NkButtonState::NK_PRESSED : NkButtonState::NK_RELEASED;
...
NkKey NkWaylandEventImpl::XkbKeyToNkKey(xkb_keysym_t sym) {
	...
	switch (sym) {
		...
		case XKB_KEY_Return:
			return NkKey::NK_RETURN;
		case XKB_KEY_Tab:
			return NkKey::NK_TAB;
		case XKB_KEY_BackSpace:
			return NkKey::NK_BACKSPACE;
		case XKB_KEY_Delete:
		...
		case XKB_KEY_Page_Up:
			return NkKey::NK_PAGEUP;
		case XKB_KEY_Page_Down:
			return NkKey::NK_PAGEDOWN;
		case XKB_KEY_Left:
