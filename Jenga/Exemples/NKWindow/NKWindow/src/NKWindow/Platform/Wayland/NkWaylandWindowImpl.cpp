...
void NkWaylandWindowImpl::Close()
{
    ...
    if (mData.toplevel)    { xdg_toplevel_destroy(mData.toplevel);    mData.toplevel    = nullptr; }
    if (mData.xdgSurface)  { xdg_surface_destroy(mData.xdgSurface);   mData.xdgSurface  = nullptr; }
    if (mData.surface)     { wl_surface_destroy(mData.surface);        mData.surface     = nullptr; }
    if (mData.wmBase)      { xdg_wm_base_destroy(mData.wmBase);        mData.wmBase      = nullptr; }
    if (mData.shm)         { wl_shm_destroy(mData.shm);                mData.shm         = nullptr; }
    if (mData.seat)        { wl_seat_destroy(mData.seat);              mData.seat        = nullptr; }
    if (mData.compositor)  { wl_compositor_destroy(mData.compositor);  mData.compositor  = nullptr; }
    if (mData.registry)    { wl_registry_destroy(mData.registry);      mData.registry    = nullptr; }
    if (mData.display)     { wl_display_disconnect(mData.display);     mData.display     = nullptr; }
...
bool NkWaylandWindowImpl::CreateShmBuffer(NkU32 w, NkU32 h)
{
    ...
    mData.buffer = wl_shm_pool_create_buffer(pool,
        0,
        static_cast<int32_t>(w),
        static_cast<int32_t>(h),
        static_cast<int32_t>(mData.stride),
...
bool        NkWaylandWindowImpl::IsOpen()       const { return mData.isOpen; }
NkError     NkWaylandWindowImpl::GetLastError() const { return mLastError; }
std::string NkWaylandWindowImpl::GetTitle()     const { return mTitle; }
NkVec2u     NkWaylandWindowImpl::GetSize()      const { return { mData.width, mData.height }; }
...
void NkWaylandWindowImpl::SetSize(NkU32 w, NkU32 h)
{
    ...
    {
        wl_surface_attach(mData.surface, mData.buffer, 0, 0);
        wl_surface_damage(mData.surface, 0, 0,
...
void NkWaylandWindowImpl::SetVisible(bool v)
{
    ...
    else if (v && mData.surface && mData.buffer)
    {
        wl_surface_attach(mData.surface, mData.buffer, 0, 0);
        wl_surface_damage(mData.surface, 0, 0,
            static_cast<int32_t>(mData.width), static_cast<int32_t>(mData.height));
        wl_surface_commit(mData.surface);
    }
}
...
void NkWaylandWindowImpl::SetFullscreen(bool fs)
{
    ...
    mData.fullscreen   = fs;
    if (!mData.toplevel) return;
    if (fs) xdg_toplevel_set_fullscreen(mData.toplevel, nullptr);
    else    xdg_toplevel_unset_fullscreen(mData.toplevel);
    if (mData.surface) wl_surface_commit(mData.surface);
}
...
void NkWaylandWindowImpl::OnRegistryGlobal(
    void* data, wl_registry* reg, uint32_t id, const char* iface, uint32_t ver)
{
    auto* self = static_cast<NkWaylandWindowImpl*>(data);

    if (strcmp(iface, wl_compositor_interface.name) == 0)
        self->mData.compositor =
            static_cast<wl_compositor*>(wl_registry_bind(reg, id, &wl_compositor_interface, 4));
    else if (strcmp(iface, wl_shm_interface.name) == 0)
        self->mData.shm =
            static_cast<wl_shm*>(wl_registry_bind(reg, id, &wl_shm_interface, 1));
    else if (strcmp(iface, xdg_wm_base_interface.name) == 0)
        self->mData.wmBase =
            static_cast<xdg_wm_base*>(wl_registry_bind(reg, id, &xdg_wm_base_interface, 1));
    else if (strcmp(iface, wl_seat_interface.name) == 0)
        self->mData.seat =
            static_cast<wl_seat*>(wl_registry_bind(reg, id, &wl_seat_interface,
                                                   (ver < 5) ? ver : 5));
    (void)ver;
}
...
void NkWaylandWindowImpl::OnXdgSurfaceConfigure(
...
void NkWaylandWindowImpl::OnXdgToplevelConfigure(
    void* data, xdg_toplevel*, int32_t w, int32_t h, wl_array*)
{
    ...
    if (w > 0 && h > 0)
    {
        ...
        if (static_cast<NkU32>(w) != self->mData.width ||
            static_cast<NkU32>(h) != self->mData.height)
        {
