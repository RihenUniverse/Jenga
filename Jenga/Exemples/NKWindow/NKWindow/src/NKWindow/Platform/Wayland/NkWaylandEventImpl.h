...
private:
    ...
    // File d'événements
    std::deque<NkEvent>  mQueue;
    NkEventCallback      mGlobalCallback;

    std::unordered_map<wl_surface*, WindowEntry> mWindowMap;
    ...
    static void OnKeyboardKey   (void* data, wl_keyboard*, uint32_t serial, uint32_t time,
                                 uint32_t key, uint32_t state);
    static void OnKeyboardMods  (void* data, wl_keyboard*, uint32_t serial,
    ...
    static void OnPointerMotion(void* data, wl_pointer*, uint32_t time,
                                wl_fixed_t sx, wl_fixed_t sy);
    static void OnPointerButton(void* data, wl_pointer*, uint32_t serial, uint32_t time,
                                uint32_t button, uint32_t state);
    static void OnPointerAxis  (void* data, wl_pointer*, uint32_t time,
