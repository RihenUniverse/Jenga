...
EM_BOOL NkWASMEventImpl::OnMouseMove(int, const EmscriptenMouseEvent *me, void *) {
	...
	NkMouseMoveData d(mapped.x, mapped.y, static_cast<NkI32>(me->screenX), static_cast<NkI32>(me->screenY),
...
EM_BOOL NkWASMEventImpl::OnMouseDown(int, const EmscriptenMouseEvent *me, void *) {
	...
	const NkCanvasCoordMap mapped =
		MapCssToCanvasCoords(static_cast<NkI32>(me->targetX), static_cast<NkI32>(me->targetY));
	NkMouseButtonData d(btn, NkButtonState::NK_PRESSED, mapped.x, mapped.y, static_cast<NkI32>(me->screenX),
						static_cast<NkI32>(me->screenY));
	sInstance->PushAndDispatch(NkEvent(NkEventType::NK_MOUSE_BUTTON_PRESS, d), sInstance->mPrimaryHandle);
	return EM_TRUE;
}
...
EM_BOOL NkWASMEventImpl::OnMouseUp(int, const EmscriptenMouseEvent *me, void *) {
	...
	const NkCanvasCoordMap mapped =
		MapCssToCanvasCoords(static_cast<NkI32>(me->targetX), static_cast<NkI32>(me->targetY));
	NkMouseButtonData d(btn, NkButtonState::NK_RELEASED, mapped.x, mapped.y, static_cast<NkI32>(me->screenX),
						static_cast<NkI32>(me->screenY));
	sInstance->PushAndDispatch(NkEvent(NkEventType::NK_MOUSE_BUTTON_RELEASE, d), sInstance->mPrimaryHandle);
	return EM_TRUE;
}
