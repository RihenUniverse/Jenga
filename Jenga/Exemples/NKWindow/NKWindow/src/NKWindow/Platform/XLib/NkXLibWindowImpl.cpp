...
bool NkXLibWindowImpl::Create(const NkWindowConfig &config) {
	...
	xa.event_mask = ExposureMask | StructureNotifyMask | KeyPressMask | KeyReleaseMask | ButtonPressMask |
					ButtonReleaseMask | PointerMotionMask | FocusChangeMask;

	mData.window = XCreateWindow(mData.display, RootWindow(mData.display, mData.screen), wx, wy, config.width,
								 config.height, 0, DefaultDepth(mData.display, mData.screen), InputOutput,
								 DefaultVisual(mData.display, mData.screen), CWBackPixel | CWEventMask, &xa);
	...
	hints.min_width = config.minWidth;
	hints.min_height = config.minHeight;
	hints.max_width = config.maxWidth;
	hints.max_height = config.maxHeight;
...
NkVec2u NkXLibWindowImpl::GetSize() const {
	...
	// Query actual geometry from the X server so the auto-resize logic in
	// NkRenderer::BeginFrame() sees true current dimensions even after the
	// window manager resizes the window externally (ConfigureNotify).
	::Window root;
	int x = 0, y = 0;
...
void NkXLibWindowImpl::SetVisible(bool v) {
	v ? XMapWindow(mData.display, mData.window) : XUnmapWindow(mData.display, mData.window);
}
...
void NkXLibWindowImpl::SetMousePosition(NkU32 x, NkU32 y) {
	XWarpPointer(mData.display, None, mData.window, 0, 0, 0, 0, x, y);
	XFlush(mData.display);
}
...
void NkXLibWindowImpl::CaptureMouse(bool cap) {
	if (cap)
		XGrabPointer(mData.display, mData.window, True, ButtonPressMask | ButtonReleaseMask | PointerMotionMask,
