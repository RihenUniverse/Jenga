...
void NkAndroidEventImpl::PushEvent(const NkEvent &e) {
	mQueue.push(e);
}
...
void NkAndroidEventImpl::OnAppCmd(android_app *app, int32_t cmd) {
	...
	switch (cmd) {
		case APP_CMD_INIT_WINDOW:
			ev = NkEvent(NkEventType::NK_WINDOW_CREATE);
			break;
		case APP_CMD_TERM_WINDOW:
			ev = NkEvent(NkEventType::NK_WINDOW_DESTROY);
			break;
		case APP_CMD_GAINED_FOCUS:
			ev = NkEvent(NkFocusData(true));
			break;
		case APP_CMD_LOST_FOCUS:
			ev = NkEvent(NkFocusData(false));
			break;
		case APP_CMD_WINDOW_RESIZED: {
			ANativeWindow *w = app->window;
			if (w)
				ev = NkEvent(NkResizeData(static_cast<NkU32>(ANativeWindow_getWidth(w)),
...
int32_t NkAndroidEventImpl::OnInputEvent(android_app *app, AInputEvent *aev) {
	...
	if (AInputEvent_getType(aev) == AINPUT_EVENT_TYPE_MOTION) {
		...
		if (k != NkKey::NK_KEY_MAX) {
			...
			NkButtonState st = (act == AKEY_EVENT_ACTION_DOWN) ? NkButtonState::NK_PRESSED : NkButtonState::NK_RELEASED;
