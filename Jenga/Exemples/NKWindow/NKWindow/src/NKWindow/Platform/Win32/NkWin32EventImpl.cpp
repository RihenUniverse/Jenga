...
// Pompe Win32, table HWND, WndProc, mapping VK/scancode â†’ NkKey.
// =============================================================================
...
thread_local std::unordered_map<HWND, NkWin32EventImpl::WindowEntry> NkWin32EventImpl::sWindowMap;

thread_local NkWin32WindowImpl *NkWin32EventImpl::sPendingOwner = nullptr;

thread_local NkWin32EventImpl *NkWin32EventImpl::sPendingEventImpl = nullptr;
...
bool NkWin32EventImpl::IsEmpty() const {
	return mQueue.empty();
}
std::size_t NkWin32EventImpl::Size() const {
	return mQueue.size();
}
void NkWin32EventImpl::PushEvent(const NkEvent &e) {
	mQueue.push(e);
}
...
LRESULT CALLBACK NkWin32EventImpl::WindowProcStatic(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
	...
	// Retrouver l'EventImpl depuis le premier sWindowMap â€” on cherche
	...
	// Le pattern est : WindowProcStatic â†’ ProcessWin32Message via sPendingEventImpl.
	// Mais aprÃ¨s bootstrap, on doit localiser l'instance.
	// Solution : stocker un backpointer dans WindowEntry.
	// â†’ On va l'ajouter dans la struct (voir ProcessWin32Message).

	// Pour l'instant, on passe par le global sPendingEventImpl
	// ou par un singleton de thread. On utilise une variable statique
	// thread_local supplÃ©mentaire qui pointe vers l'EventImpl active.
	static thread_local NkWin32EventImpl *sCurrentImpl = nullptr;

	// Au RegisterPending on sauvegarde aussi sCurrentImpl
	if (sPendingEventImpl)
		sCurrentImpl = sPendingEventImpl;

	if (!sCurrentImpl)
		return DefWindowProc(hwnd, msg, wp, lp);

	return sCurrentImpl->ProcessWin32Message(hwnd, msg, wp, lp, it->second.window);
}
...
LRESULT NkWin32EventImpl::ProcessWin32Message(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp, NkWin32WindowImpl *owner) {
	LRESULT result = 0;
	NkEvent nkEvent;
	...
	switch (msg) {
		...
		case WM_CREATE:
			nkEvent = NkEvent(
				NkWindowCreateData(owner ? owner->GetConfig().width : 0, owner ? owner->GetConfig().height : 0));
			break;
		...
		case WM_PAINT: {
			...
			RECT rc;
			GetClientRect(hwnd, &rc);
			nkEvent = NkEvent(NkWindowPaintData(0, 0, static_cast<NkU32>(rc.right - rc.left),
		...
		// -------------------------------------------------------------------
		case WM_SETFOCUS:
			nkEvent = NkEvent(NkWindowFocusData(true));
			break;
		case WM_KILLFOCUS:
			nkEvent = NkEvent(NkWindowFocusData(false));
			break;
			...
			if (sz > 0) {
				...
				if (GetRawInputData((HRAWINPUT)lp, RID_INPUT, buf.data(), &sz, sizeof(RAWINPUTHEADER)) == sz) {
					...
					if (raw->header.dwType == RIM_TYPEMOUSE)
						nkEvent = NkEvent(NkMouseRawData(raw->data.mouse.lLastX, raw->data.mouse.lLastY, 0));
				}
			}
			break;
		}
...
#define NK_MB(Btn, State)                                                                                              \
	...
	break
		...
		case WM_LBUTTONDOWN:
			NK_MB(NK_MB_LEFT, NK_PRESSED);
		case WM_LBUTTONUP:
			NK_MB(NK_MB_LEFT, NK_RELEASED);
		case WM_RBUTTONDOWN:
			NK_MB(NK_MB_RIGHT, NK_PRESSED);
		case WM_RBUTTONUP:
			NK_MB(NK_MB_RIGHT, NK_RELEASED);
		case WM_MBUTTONDOWN:
			NK_MB(NK_MB_MIDDLE, NK_PRESSED);
		case WM_MBUTTONUP:
			NK_MB(NK_MB_MIDDLE, NK_RELEASED);
#undef NK_MB
		...
		case WM_XBUTTONUP: {
			...
			SHORT xb = HIWORD(wp);
			NkMouseButtonData d;
			d.button = (xb & XBUTTON1) ? NkMouseButton::NK_MB_BACK : NkMouseButton::NK_MB_FORWARD;
			d.state = (msg == WM_XBUTTONDOWN) ? NkButtonState::NK_PRESSED : NkButtonState::NK_RELEASED;
			d.x = pt.x;
			d.y = pt.y;
			d.screenX = sp.x;
			d.screenY = sp.y;
		...
		case WM_SYSKEYUP: {
			...
			// Conversion scancode PS/2 â†’ NkKey (position physique)
			NkKey k = NkKeycodeMap::NkKeyFromWin32Scancode(win32Sc, isExt);
			// Fallback VK
			if (k == NkKey::NK_UNKNOWN)
				k = NkKeycodeMap::NkKeyFromWin32VK(static_cast<NkU32>(wp), isExt);

			if (k != NkKey::NK_UNKNOWN) {
				bool isPress = (msg == WM_KEYDOWN || msg == WM_SYSKEYDOWN);
				NkButtonState st = isPress ? (isRep ? NkButtonState::NK_REPEAT : NkButtonState::NK_PRESSED)
				...
				NkEventType et = isPress ? (isRep ? NkEventType::NK_KEY_REPEAT : NkEventType::NK_KEY_PRESS)
		...
		case WM_MOVE:
			nkEvent = NkEvent(NkWindowMoveData(static_cast<NkI32>(LOWORD(lp)), static_cast<NkI32>(HIWORD(lp))));
			break;
