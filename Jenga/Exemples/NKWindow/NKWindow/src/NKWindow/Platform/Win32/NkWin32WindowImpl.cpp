...
// CrÃ©ation fenÃªtre Win32 Unicode (RegisterClassExW / CreateWindowExW).
// Toutes les chaÃ®nes std::string sont converties en WCHAR via NkUtf8ToWide().
// =============================================================================
...
bool NkWin32WindowImpl::Create(const NkWindowConfig &config) {
	...
	// --- Bootstrap EventImpl ---
	NkWin32EventImpl *ev = static_cast<NkWin32EventImpl *>(NkGetEventImpl());
	if (ev)
		ev->RegisterPending(this);
	...
	RegisterClassExW(&wc); // peut Ã©chouer si dÃ©jÃ  enregistrÃ©e
	...
	// --- CrÃ©ation Unicode ---
	mData.hwnd = CreateWindowExW(0, wClassName.c_str(), wTitle.c_str(), mData.dwStyle, 0, 0, rc.right - rc.left,
								 rc.bottom - rc.top, nullptr, nullptr, mData.hinstance, nullptr);

	NkSetThreadDpiAwarenessContext(prev);

	if (!mData.hwnd) {
		mLastError = NkError(1, "CreateWindowExW failed (" + std::to_string(::GetLastError()) + ")");
		return false;
	}
	...
	if (!config.fullscreen) {
		...
		if (config.centered)
			SetWindowPos(mData.hwnd, nullptr, (sw - ww) / 2, (sh - wh) / 2, ww, wh, SWP_NOZORDER);
		else
			SetWindowPos(mData.hwnd, nullptr, config.x, config.y, ww, wh, SWP_NOZORDER);
	}
	...
	// Enregistrement final dans l'EventImpl (aprÃ¨s HWND valide)
	if (ev)
		ev->Initialize(this, mData.hwnd);
...
bool NkWin32WindowImpl::IsOpen() const {
	return mData.isOpen;
}
NkError NkWin32WindowImpl::GetLastError() const {
	return mLastError;
}
...
NkVec2u NkWin32WindowImpl::GetSize() const {
	RECT rc = {};
	if (mData.hwnd)
		GetClientRect(mData.hwnd, &rc);
	return {static_cast<NkU32>(rc.right - rc.left), static_cast<NkU32>(rc.bottom - rc.top)};
}
...
float NkWin32WindowImpl::GetDpiScale() const {
	return mData.hwnd ? static_cast<float>(NkGetDpiForWindow(mData.hwnd)) / USER_DEFAULT_SCREEN_DPI : 1.f;
}

NkVec2u NkWin32WindowImpl::GetDisplaySize() const {
	return {static_cast<NkU32>(GetSystemMetrics(SM_CXSCREEN)), static_cast<NkU32>(GetSystemMetrics(SM_CYSCREEN))};
}
...
void NkWin32WindowImpl::SetSize(NkU32 w, NkU32 h) {
	...
	AdjustWindowRectEx(&rc, mData.dwStyle, FALSE, mData.dwExStyle);
	SetWindowPos(mData.hwnd, nullptr, 0, 0, rc.right - rc.left, rc.bottom - rc.top, SWP_NOMOVE | SWP_NOZORDER);
}
...
void NkWin32WindowImpl::SetFullscreen(bool fs) {
	if (fs) {
		SetWindowLongW(mData.hwnd, GWL_STYLE, WS_POPUP | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN);
		SetWindowPos(mData.hwnd, HWND_TOP, 0, 0, GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN),
					 SWP_FRAMECHANGED);
	} else {
		SetWindowLongW(mData.hwnd, GWL_STYLE, static_cast<LONG>(mData.dwStyle));
		SetWindowPos(mData.hwnd, nullptr, mConfig.x, mConfig.y, static_cast<int>(mConfig.width),
					 static_cast<int>(mConfig.height), SWP_FRAMECHANGED | SWP_NOZORDER);
	}
...
void NkWin32WindowImpl::SetProgress(float progress) {
	if (mData.taskbarList) {
		...
		mData.taskbarList->SetProgressValue(mData.hwnd, static_cast<ULONGLONG>(progress * kMax), kMax);
	}
}
