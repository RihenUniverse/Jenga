...
- (NSDragOperation)draggingEntered:(id<NSDraggingInfo>)sender
{
    ...
    if (self.callbacks && self.callbacks->onEnter) self.callbacks->onEnter(d);
    return NSDragOperationCopy;
}

- (void)draggingExited:(id<NSDraggingInfo>)sender
{
    if (self.callbacks && self.callbacks->onLeave) self.callbacks->onLeave();
}
...
- (BOOL)performDragOperation:(id<NSDraggingInfo>)sender
{
    ...
    if (urls.count > 0 && self.callbacks && self.callbacks->onFiles)
    {
        ...
        for (NSURL* url in urls)
        {
            ...
            if (p) strncpy(fp.path, p, sizeof(fp.path) - 1);
