...
void NkXCBEventImpl::PushEvent(const NkEvent &e) {
	mQueue.push(e);
}
		...
		switch (type) {
			...
			case XCB_KEY_RELEASE: {
				...
				if (k != NkKey::NK_UNKNOWN) {
					NkButtonState st = (type == XCB_KEY_PRESS) ? NkButtonState::NK_PRESSED : NkButtonState::NK_RELEASED;
					NkKeyData kd;
					kd.key = k;
					kd.state = st;
			...
			case XCB_BUTTON_RELEASE: {
				...
				srcWindow = be->event;
				NkButtonState st = (type == XCB_BUTTON_PRESS) ? NkButtonState::NK_PRESSED : NkButtonState::NK_RELEASED;
				switch (be->detail) {
					case 1: {
					...
					case 4:
						if (type == XCB_BUTTON_PRESS) {
							...
							wd.delta = 1.0;
							wd.deltaY = 1.0;
							nkEv = NkEvent(NkEventType::NK_MOUSE_WHEEL_VERTICAL, wd);
						}
						break;
					case 5:
						if (type == XCB_BUTTON_PRESS) {
			...
			case XCB_MOTION_NOTIFY: {
				...
				md.x = me->event_x;
				md.y = me->event_y;
				md.screenX = me->root_x;
				md.screenY = me->root_y;
				nkEv = NkEvent(md);
				break;
			}
