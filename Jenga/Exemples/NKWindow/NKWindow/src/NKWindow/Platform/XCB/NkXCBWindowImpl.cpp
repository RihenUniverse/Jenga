...
// Create() appelle eventImpl.Initialize(this, &mData.window)
// Close()  appelle eventImpl.Shutdown(&mData.window)
// =============================================================================
...
static xcb_atom_t InternAtom(xcb_connection_t *c, const char *name, bool onlyIfExists = false) {
	xcb_intern_atom_cookie_t ck = xcb_intern_atom(c, onlyIfExists ? 1 : 0, static_cast<uint16_t>(strlen(name)), name);
...
bool NkXCBWindowImpl::Create(const NkWindowConfig &config) {
	...
	NkU32 bg = (r << 16) | (g << 8) | b, fg = 0xFFFFFF;
	...
	uint32_t vals[] = {bg, XCB_EVENT_MASK_EXPOSURE | XCB_EVENT_MASK_STRUCTURE_NOTIFY | XCB_EVENT_MASK_KEY_PRESS |
							   XCB_EVENT_MASK_KEY_RELEASE | XCB_EVENT_MASK_BUTTON_PRESS |
							   XCB_EVENT_MASK_BUTTON_RELEASE | XCB_EVENT_MASK_POINTER_MOTION |
							   XCB_EVENT_MASK_FOCUS_CHANGE};

	xcb_create_window(mData.connection, XCB_COPY_FROM_PARENT, mData.window, mData.screen->root,
					  static_cast<int16_t>(wx), static_cast<int16_t>(wy), static_cast<uint16_t>(config.width),
					  static_cast<uint16_t>(config.height), 0, XCB_WINDOW_CLASS_INPUT_OUTPUT, mData.screen->root_visual,
					  mask, vals);

	xcb_change_property(mData.connection, XCB_PROP_MODE_REPLACE, mData.window, XCB_ATOM_WM_NAME, XCB_ATOM_STRING, 8,
						static_cast<uint32_t>(config.title.size()), config.title.c_str());

	mData.wmProtocols = InternAtom(mData.connection, "WM_PROTOCOLS");
	mData.wmDelete = InternAtom(mData.connection, "WM_DELETE_WINDOW");
	xcb_change_property(mData.connection, XCB_PROP_MODE_REPLACE, mData.window, mData.wmProtocols, XCB_ATOM_ATOM, 32, 1,
						&mData.wmDelete);
	...
	xcb_create_cursor(mData.connection, mData.blankCursor, pix, pix, 0, 0, 0, 0, 0, 0, 0, 0);
	...
	// Enregistre la fenêtre dans l'EventImpl
...
void NkXCBWindowImpl::Close() {
	if (!mData.isOpen)
		return;
	// Désenregistre d'abord
	if (mEventImpl)
		mEventImpl->Shutdown(&mData.window);
...
bool NkXCBWindowImpl::IsOpen() const {
	return mData.isOpen;
}
NkError NkXCBWindowImpl::GetLastError() const {
	return mLastError;
}
std::string NkXCBWindowImpl::GetTitle() const {
	return mConfig.title;
}
NkU32 NkXCBWindowImpl::GetBackgroundColor() const {
	return mBgColor;
}
void NkXCBWindowImpl::SetBackgroundColor(NkU32 c) {
	mBgColor = c;
}
...
void NkXCBWindowImpl::SetSize(NkU32 w, NkU32 h) {
	...
	mData.height = h;
	uint32_t v[] = {w, h};
	xcb_configure_window(mData.connection, mData.window, XCB_CONFIG_WINDOW_WIDTH | XCB_CONFIG_WINDOW_HEIGHT, v);
	xcb_flush(mData.connection);
}

void NkXCBWindowImpl::SetPosition(NkI32 x, NkI32 y) {
	uint32_t v[] = {static_cast<uint32_t>(x), static_cast<uint32_t>(y)};
	xcb_configure_window(mData.connection, mData.window, XCB_CONFIG_WINDOW_X | XCB_CONFIG_WINDOW_Y, v);
	xcb_flush(mData.connection);
}

void NkXCBWindowImpl::SetVisible(bool v) {
	v ? xcb_map_window(mData.connection, mData.window) : xcb_unmap_window(mData.connection, mData.window);
	xcb_flush(mData.connection);
}
...
void NkXCBWindowImpl::SetMousePosition(NkU32 x, NkU32 y) {
	xcb_warp_pointer(mData.connection, XCB_NONE, mData.window, 0, 0, 0, 0, static_cast<int16_t>(x),
...
void NkXCBWindowImpl::ShowMouse(bool show) {
	xcb_change_window_attributes(mData.connection, mData.window, XCB_CW_CURSOR,
								 show ? (const uint32_t[]){XCB_CURSOR_NONE} : &mData.blankCursor);
	xcb_flush(mData.connection);
}
...
void NkXCBWindowImpl::CaptureMouse(bool cap) {
	if (cap)
		xcb_grab_pointer(mData.connection, 1, mData.window,
						 ...
						 XCB_GRAB_MODE_ASYNC, XCB_GRAB_MODE_ASYNC, mData.window, XCB_NONE, XCB_CURRENT_TIME);
	else
...
void NkXCBWindowImpl::BlitSoftwareFramebuffer(const NkU8 *rgba8, NkU32 w, NkU32 h) {
	...
	xcb_put_image(mData.connection, XCB_IMAGE_FORMAT_Z_PIXMAP, mData.window, mData.gc, static_cast<uint16_t>(w),
				  static_cast<uint16_t>(h), 0, 0, 0, mData.screen->root_depth, static_cast<uint32_t>(bgrx.size()),
				  bgrx.data());
	xcb_flush(mData.connection);
}
