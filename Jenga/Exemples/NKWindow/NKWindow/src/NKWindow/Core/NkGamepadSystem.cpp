...
void NkGamepadSystem::Rumble(NkU32 idx, float motorLow, float motorHigh, float triggerLeft, float triggerRight,
							 NkU32 durationMs) {
	if (mReady && mBackend) {
		mBackend->Rumble(idx, motorLow, motorHigh, triggerLeft, triggerRight, durationMs);

		if (IEventImpl *eventImpl = NkGetEventImpl()) {
			NkEvent event(NkGamepadRumbleData{idx, motorLow, motorHigh, triggerLeft, triggerRight, durationMs});
			eventImpl->PushEvent(event);
		}
	}
}
...
void NkGamepadSystem::FireConnect(const NkGamepadInfo &info, bool connected) {
	if (IEventImpl *eventImpl = NkGetEventImpl()) {
		NkEvent event(NkGamepadConnectData(connected, info));
		eventImpl->PushEvent(event);
	}
...
void NkGamepadSystem::FireButton(NkU32 idx, NkGamepadButton btn, NkButtonState st) {
	if (IEventImpl *eventImpl = NkGetEventImpl()) {
		NkEvent event(NkGamepadButtonData(idx, btn, st));
		eventImpl->PushEvent(event);
	}
...
void NkGamepadSystem::FireAxis(NkU32 idx, NkGamepadAxis ax, float value, float prevValue) {
	if (IEventImpl *eventImpl = NkGetEventImpl()) {
		NkEvent event(NkGamepadAxisData(idx, ax, value, prevValue));
		eventImpl->PushEvent(event);
	}
