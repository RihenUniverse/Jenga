...
Window::Window() : mImpl(std::make_unique<PlatformWindowImpl>()) {
}

Window::Window(const NkWindowConfig &config) : mImpl(std::make_unique<PlatformWindowImpl>()), mConfig(config) {
	Create(config);
}
...
void Window::SetSize(NkU32 w, NkU32 h) {
	if (mImpl)
		mImpl->SetSize(w, h);
}
void Window::SetPosition(NkI32 x, NkI32 y) {
	if (mImpl)
...
void Window::SetMousePosition(NkU32 x, NkU32 y) {
	if (mImpl)
...
void Window::CaptureMouse(bool cap) {
	if (mImpl)
...
void Window::SetProgress(float p) {
	if (mImpl)
		mImpl->SetProgress(p);
}
