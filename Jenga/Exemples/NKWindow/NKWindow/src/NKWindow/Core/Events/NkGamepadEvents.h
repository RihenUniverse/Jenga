...
struct NkGamepadConnectData {
	...
	std::string ToString() const {
		return std::string(connected ? "GamepadConnect" : "GamepadDisconnect") + "(idx=" + std::to_string(info.index) +
			   " \"" + info.id + "\")";
	}
};
...
struct NkGamepadButtonData {
	...
	std::string ToString() const {
		return "GamepadButton(#" + std::to_string(gamepadIndex) + " " + NkGamepadButtonToString(button) + " " +
...
struct NkGamepadAxisData {
	...
	NkGamepadAxisData(NkU32 idx, NkGamepadAxis a, float v, float pv, float dz = 0.05f)
		: gamepadIndex(idx), axis(a), value(v), prevValue(pv), delta(v - pv), deadzone(dz) {
	}
	...
	std::string ToString() const {
		return "GamepadAxis(#" + std::to_string(gamepadIndex) + " " + NkGamepadAxisToString(axis) +
			   " value=" + std::to_string(value) + " delta=" + std::to_string(delta) + ")";
	}
};
...
struct NkGamepadStateData {
	...
	bool IsButtonDown(NkGamepadButton b) const {
		...
		return (idx < static_cast<NkU32>(NkGamepadButton::NK_GAMEPAD_BUTTON_MAX)) && buttons[idx];
	}

	float GetAxis(NkGamepadAxis a) const {
		NkU32 idx = static_cast<NkU32>(a);
		return (idx < static_cast<NkU32>(NkGamepadAxis::NK_GAMEPAD_AXIS_MAX)) ? axes[idx] : 0.f;
	}

	std::string ToString() const {
		return "GamepadState(#" + std::to_string(gamepadIndex) + (connected ? " connected)" : " disconnected)");
	}
};
...
struct NkGamepadRumbleData {
	...
	NkGamepadRumbleData(NkU32 idx, float low, float high, float tl = 0.f, float tr = 0.f, NkU32 dur = 0)
		: gamepadIndex(idx), motorLow(low), motorHigh(high), triggerLeft(tl), triggerRight(tr), durationMs(dur) {
	}

	std::string ToString() const {
		return "GamepadRumble(#" + std::to_string(gamepadIndex) + " L=" + std::to_string(motorLow) +
