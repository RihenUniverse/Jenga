...
struct NkKeyData {
	...
	NkKeyData() = default;
	NkKeyData(NkKey k, NkButtonState s, const NkModifierState &m, NkScancode sc = NkScancode::NK_SC_UNKNOWN,
			  NkU32 native = 0, bool ext = false, bool rep = false)
		: key(k), state(s), modifiers(m), scancode(sc), nativeKey(native), extended(ext), repeat(rep) {
	}
	...
	bool IsRepeat() const {
		return state == NkButtonState::NK_REPEAT;
	}
	bool IsModifierKey() const {
		return NkKeyIsModifier(key);
	}
...
inline NkTextInputData::NkTextInputData(NkU32 cp) : NkTextInputData(NkTextInputData::FromCodepoint(cp)) {
}
