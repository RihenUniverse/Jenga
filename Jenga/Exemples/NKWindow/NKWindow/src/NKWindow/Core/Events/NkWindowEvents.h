...
struct NkWindowMoveData {
	...
	NkWindowMoveData(NkI32 x, NkI32 y, NkI32 px = 0, NkI32 py = 0) : x(x), y(y), prevX(px), prevY(py) {
	}

	std::string ToString() const {
		return "WindowMove(" + std::to_string(prevX) + "," + std::to_string(prevY) + " -> " + std::to_string(x) + "," +
			   std::to_string(y) + ")";
	}
};
...
struct NkWindowDpiData {
	...
	NkWindowDpiData(float scale, float prev, NkU32 dpi) : scale(scale), prevScale(prev), dpi(dpi) {
	}

	std::string ToString() const {
		return "WindowDpi(" + std::to_string(prevScale) + " -> " + std::to_string(scale) + ", " + std::to_string(dpi) +
			   "dpi)";
	}
};
	...
	std::string ToString() const {
		switch (theme) {
			...
			case NkWindowTheme::NK_THEME_HIGH_CONTRAST:
				return "WindowTheme(HIGH_CONTRAST)";
			default:
...
struct NkWindowStateData {
	...
	enum class State : NkU32 { Minimized, Maximized, Restored, Fullscreen, Windowed } state = State::Restored;
...
struct NkWindowPaintData {
	...
	NkWindowPaintData(NkI32 x, NkI32 y, NkU32 w, NkU32 h) : dirtyX(x), dirtyY(y), dirtyW(w), dirtyH(h) {
	}
	...
	std::string ToString() const {
		...
		return "WindowPaint(" + std::to_string(dirtyX) + "," + std::to_string(dirtyY) + " " + std::to_string(dirtyW) +
			   "x" + std::to_string(dirtyH) + ")";
	}
};
